...
def calculate_demographic_data(print_data=True):
    ...
    # What percentage of the people who work with the minimum number of hours per week have a salary of '>50K'
    num_min_workers=df[df['hours-per-weeek'] == min_work_hours].shape[0]
    rich_percentage=round((float(df[(df['hours-per-week'] == min_work_hours) & (df['salary']=='>50K')].shape[0])/num_min_workers)*100,1)

    # What country has the highest percentage of poeple that earn > 50K?
    highest_earning_country=''
    highest_earning_country_percentage = 0
    for (country), sub_df in df.groupby(
        ['native-country']):
